...
class BrewingStandMenu extends BrewingStandInventory implements BaseMenu {
	use MenuTrait {
		...
		if (!isset($this->isSent[$player->getXuid()])) {
			...
			$player->getNetworkSession()->sendDataPacket(
				UpdateBlockPacket::create(
					BlockPosition::fromVector3($this->holder),
					TypeConverter::getInstance()->getBlockTranslator()->internalIdToNetworkId(VanillaBlocks::BREWING_STAND()->getStateId()),
					UpdateBlockPacket::FLAG_NETWORK,
					UpdateBlockPacket::DATA_LAYER_NORMAL
				)
			);
